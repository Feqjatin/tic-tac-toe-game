...
async function fetchGameState() {
    ...
    try {
        const response = await fetch(`https://tic-tac-toe-game-e4oh.onrender.com/gameState?uid=${localStorage.getItem('uid')}&sid=${localStorage.getItem('uCode')}`, {
            method: 'GET',
            headers: {
...
async function makeMove(position) {
    try {
        const response = await fetch('https://tic-tac-toe-game-e4oh.onrender.com/makeMove', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function restartGame() {
    closePopup();
    try {
        const response = await fetch('https://tic-tac-toe-game-e4oh.onrender.com/restartGame', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
